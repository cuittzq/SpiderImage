package cn.tzq.spider.util.http;

import cn.tzq.spider.exception.HttpException;
import cn.tzq.spider.util.Jsons;
import cn.tzq.spider.util.Strings;
...
public class Http {
    ...
    private void setOptionalHeaders(HttpRequest request) {
        if (Strings.notEmpty(contentType)) {
            request.contentType(contentType, charset);
        }
        if (Strings.notEmpty(accept)) {
